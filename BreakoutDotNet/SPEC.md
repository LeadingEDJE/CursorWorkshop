...
The existing project (`BreakoutGame.csproj`) is a .NET 10 Blazor WASM standalone app with Bootstrap 5, a placeholder `<canvas>` on `Pages/Home.razor`, and no game logic yet.
...
requestAnimationFrame (JS)
  -> tick(deltaTime, inputState) call into C# [JSInvokable]
    -> C# updates physics, collisions, game state
    -> C# returns RenderFrame (list of draw commands)
  -> JS clears canvas, executes draw commands
  -> JS calls requestAnimationFrame (loop)
```
...
```
[Start] --> Title
Title --Space--> ReadyToLaunch
ReadyToLaunch --Space--> Playing
Playing --"All bricks cleared"--> LevelComplete
Playing --"Ball below paddle"--> LifeLost
LifeLost --"After 1s pause (lives > 0)"--> ReadyToLaunch
LifeLost --"Lives == 0"--> GameOver
LevelComplete --"After 2s delay"--> ReadyToLaunch
GameOver --Space--> Title
```
